...
def _get_cached_ui_word(word: str, target_lang: str) -> str:
    """Cache common UI words to avoid redundant external API calls."""
    try:
        return _translate_chunk(word, target_lang).strip()
    except Exception:
        return word
...
def _translate_chunk(text: str, target_lang: str) -> str:
    ...
    # If translation fails or returns empty, return original text as fallback
...
def translate_diagnosis_message(text: str, target_lang: str = "hi") -> dict[str, str]:
    ...
    # Create target language question header using cached UI words
    question_word = _get_cached_ui_word("Question", target_lang)
    ...
    if translated_reason:
        # Add reason as separate line, using cached "Reason:" prefix
        reason_label = _get_cached_ui_word("Reason:", target_lang)
        result_parts.append(f"{reason_label} {translated_reason}")
